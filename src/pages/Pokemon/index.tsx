import React, { useState, useEffect, SVGProps, useMemo, useCallback } from 'react';
...
const Pokemon: React.FC = () => {
  ...
  const [bookmarks, setBookmaarks] = React.useState<PokemonProps[]>(
    getStorage('books')
  );

  const setStore = (item: PokemonProps) => {
    const items: PokemonProps[] = getStorage('mypokemon');
    window.localStorage.setItem('mypokemon', JSON.stringify([item, ...items]));
    setBookmaarks([item, ...items]);
  };
      ...
      <Content>
        <Header>
          ...
          <button className="btn-action"
              key={pokemon.specie}
              onClick={() => setStore(pokemon)}>Add Pokemon
            </button>
        </Header>
